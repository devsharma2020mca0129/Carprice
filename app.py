...
def main():
    aaast.title("Car Price Prediction 🚗")
    aaast.markdown(" So let's try evaluating the price.. 🤖 ")
    ...
    if aaast.button("Estimate Price", key='predict'):
        try:
            ...
            aaaprediction = aaaModel.predict([[aaaPresent_Price, aaaKms_Driven, aaaOwner, aaaYears_old, aaaFuel_Type_Diesel, aaaFuel_Type_Petrol, aaaSeller_Type_Individual, aaaTransmission_Mannual]])
            aaaoutput = round(aaaprediction[0],2)
            if aaaoutput<0:
